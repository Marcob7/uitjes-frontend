...
export default function HomeSceneSection({
  ...
  return <section ref={rootRef} className={cx("relative isolate min-h-[700px] w-full overflow-hidden bg-[#f3cbb8] lg:min-h-[720px] xl:min-h-[780px]", className)} style={{ color: currentTheme.ink, "--sc-ink": currentTheme.ink, "--sc-muted-ink": currentTheme.mutedInk, "--sc-accent": currentTheme.accent } as CSSProperties} aria-labelledby={titleId}>
    ...
    <div className="absolute inset-x-0 bottom-5 z-30 flex justify-center px-4 sm:bottom-7 lg:bottom-8"><div className="flex max-w-full items-center gap-1 rounded-full border border-white/25 bg-white/22 p-1.5 shadow-lg shadow-black/10 backdrop-blur-xl" role="group" aria-label="Kies een moment van de dag">{PHASES.map((item, index) => { const selected = phase === index; return <button key={item.label} type="button" aria-pressed={selected} aria-label={item.ariaLabel} onClick={() => setSceneProgress(index as ScenePhase)} className={cx("relative min-h-10 rounded-full px-4 text-sm font-semibold transition", "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white", selected ? "text-white" : "text-[var(--sc-ink)] hover:bg-white/20")}>{selected && <motion.span layoutId={`active-scene-phase-${titleId}`} className="absolute inset-0 -z-10 rounded-full bg-[var(--sc-ink)]" transition={{ type: prefersReducedMotion ? "tween" : "spring", stiffness: 420, damping: 32 }} />}<span className="relative">{item.label}</span></button>; })}</div></div>
  </section>;
}
