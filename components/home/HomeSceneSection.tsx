...
const PHASES: SceneTheme[] = [
  { skyFrom: "#f8d6ba", skyVia: "#f4c6b4", skyTo: "#9cc7cf", ink: "#18343a", mutedInk: "rgba(24, 52, 58, 0.76)", glow: "rgba(255, 237, 186, 0.68)", accent: "#f07b5d" },
  { skyFrom: "#a9d6dc", skyVia: "#79bcc8", skyTo: "#4d8796", ink: "#102f36", mutedInk: "rgba(16, 47, 54, 0.76)", glow: "rgba(255, 244, 191, 0.56)", accent: "#1f6f78" },
  { skyFrom: "#152a46", skyVia: "#243454", skyTo: "#101827", ink: "#f6f3ea", mutedInk: "rgba(246, 243, 234, 0.74)", glow: "rgba(112, 137, 191, 0.32)", accent: "#f0b27a" },
];
...
export default function HomeSceneSection({
  ...
  return <section ref={rootRef} className={cx("relative isolate min-h-screen min-h-[100svh] min-h-[100dvh] w-full overflow-hidden bg-[#b8c5c3] lg:h-screen lg:h-[100dvh]", className)} style={{ color: currentTheme.ink, "--sc-ink": currentTheme.ink, "--sc-muted-ink": currentTheme.mutedInk, "--sc-accent": currentTheme.accent } as CSSProperties} aria-labelledby={titleId}>
    ...
    <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[62%]">
      ...
      {phase !== 2 && BIRDS.map((bird) => <div key={bird.left} className="absolute hidden text-[var(--sc-ink)]/50 min-[430px]:block" style={{ left: bird.left, top: bird.top, scale: bird.scale }}><Bird className="w-9 lg:w-11" /></div>)}
    </div>
    ...
    <div className="relative z-20 mx-auto flex min-h-[100svh] min-h-[100dvh] w-full max-w-[1440px] flex-col px-4 pt-20 sm:px-8 sm:pt-24 md:px-10 lg:h-full lg:min-h-0 lg:flex-row lg:items-center lg:px-14 lg:pb-36 lg:pt-16 xl:px-20"><div className="flex w-full max-w-[660px] flex-1 flex-col lg:w-[46%] lg:flex-none">
      ...
      <div className="w-full text-center">
        <div className="relative mt-[clamp(3rem,10svh,4.5rem)] flex min-h-[clamp(6.5rem,13vw,12rem)] w-full justify-center lg:static lg:mt-0">
          <div className="w-full lg:absolute lg:left-1/2 lg:top-[30%] lg:w-[min(70rem,calc(100vw-6rem))] lg:-translate-x-1/2 lg:-translate-y-1/2"><HomeWordmark titleId={titleId} prefersReducedMotion={prefersReducedMotion} isEvening={phase === 2} /></div>
        </div>
        <motion.p className="relative z-30 mx-auto mt-4 w-full max-w-[34rem] text-center text-[clamp(1rem,1.5vw,1.2rem)] leading-7 sm:mt-7 sm:leading-8 lg:absolute lg:left-1/2 lg:top-[calc(30%+8.25rem)] lg:w-[min(34rem,calc(100vw-3rem))] lg:max-w-none lg:-translate-x-1/2 lg:overflow-visible lg:text-[20.3175px]" style={{ color: currentTheme.mutedInk }} animate={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 18 }} transition={{ duration: prefersReducedMotion ? 0 : 0.65, delay: prefersReducedMotion ? 0 : 0.12 }}>{description}</motion.p>
      </div>
