...
export function WebGLLiquid({
  ...
  const content = (title || subtitle || description || children) && (
    <div
      className={cn(
        "relative z-10 mx-auto w-full max-w-[1240px] px-6 py-20 md:px-10 md:py-28",
        contentContainerClassName,
      )}
    >
      {(title || subtitle || description) && (
        <div className={cn("max-w-[760px]", contentInnerClassName)}>
          {title && (
            <h1 className={cn(LIQUID_HEADLINE_CLASS, titleClassName)}>{title}</h1>
          )}
          {subtitle && (
            <h2
              className={cn(
                "mt-2 text-[11cqi] md:text-[7cqi] lg:text-[5.5cqi] leading-[0.9] tracking-[-0.03em] font-bold text-white/95",
                subtitleClassName,
              )}
            >
              {subtitle}
          ...
          {description && (
            <p
              className={cn(
                "mt-6 max-w-[620px] text-base leading-relaxed text-white/75 md:text-xl",
                descriptionClassName,
              )}
            >
              {description}
      ...
      {children && (
        <div className={cn("mt-10", childrenContainerClassName)}>{children}</div>
      )}
    </div>
  );
  ...
  return (
    <WebGLErrorBoundary fallback={fallbackContent}>
      ...
      >
        ...
        <div
          className={cn(
            "pointer-events-none absolute inset-0 bg-gradient-to-r from-black/35 via-black/15 to-transparent",
            overlayClassName,
          )}
        />
        <div
          className={cn(
            "pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_65%_40%,rgba(255,255,255,0.16),transparent_45%)]",
            glowClassName,
          )}
        />
